...
double ray_intersect_cone(Ray ray, Shape *cone) {
    // TODO
    return INFINITY;
}
...
Intersection ray_intersect_group(Ray ray, Group group)
{
    ...
    Shape *shape_min = NULL;
    for (size_t i = 0; i < group.num_children; i++) {
        Shape *shape = &group.children[i];
        double t = ray_intersect_shape(ray, shape);
        if (t < t_min) {
            t_min = t;
            shape_min = shape;
        }
    }
    return (Intersection) { t_min, shape_min };
}
...
Intersection ray_intersect_world(Ray ray, World world)
{
    ...
    for (size_t i = 0; i < world.group_count; i++) {
        Intersection intersection = ray_intersect_group(ray, world.groups[i]);
...
Color ray_color(Ray ray, World world, int remaining_reflections) {
    ...
    if (h.t >= INFINITY || h.object_ptr == NULL) {
        return color_black();
    }
